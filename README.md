...
Its main properties and responsibilities are the following:
   ...
   * If a wrong room `id` (e.g. nonexistent room) is used to modify the `Building`, the intent is ignored and the same `Building` is returned unmodified. *Note: this behaviour could be changed to returning an error, if considered more convenient/correct*
   ...
   * Each room has an `id`, a `description` (these two sometimes grouped as `RoomInfo` for convenience) and a set of `Neighbour`s. A `Neighbour` is a tuple of room's `id` and `Direction` (North, South, East or West).
...
In this version of the application (game) the `Player` is actually just a small container of mutable details. It is kept totally encapsulated in the `Game` and has trivial semantic and logic. Having only one player, the `Game` can perfectly undertake the responsibility of communicating in its name, if necessary.
...
In a production ready application, I would take away from code all the `String`s, like messages, names, tags, etc., placing them in some specific configuration files (e.b. \*.properties).
